...
class Archivo(Elemento):
    ...
    def tiene_ext(self, ext):
        """
        Regresa True si el archivo termina en ext, de lo contrario regresa False, si
        ext es "" entonces siempre regresa True.
        """
        if ext == "":
            return True
        elif os.path.splitext(self.nombre)[1] == ext:
            return True
        else:
            return False
...
class Carpeta(Elemento):
    ...
    def obtener_elementos(self, ext=""):
        """
...
def imprimir_elementos(carpeta, ext="", nivel=0):
    """
    ...
    for e in carpeta.elementos:
        if isinstance(e, Carpeta):
            e.obtener_elementos(ext=ext)
            imprimir_elementos(e, ext, nivel+1)
        else:
            e.nivel = nivel
...
def main():
    ...
    carpeta.obtener_elementos(ext=ext)
    
    imprimir_elementos(carpeta, ext)
